...
notes:
  ...
  - UpstreamParityTest compares generated Markdown with a local upstream `dist/main.js` only when the checkout is version `1.0.1`.
  - A local upstream root can be supplied with `-DmikuTextBundle.upstreamRoot=/path/to/miku-text-bundle`.
  - The parity test is skipped when Node or a local upstream 1.0.1 build is unavailable.
```
